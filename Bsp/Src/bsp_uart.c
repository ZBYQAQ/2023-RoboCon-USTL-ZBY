...
/***********************************************************************/
void USART_1_init(uint8_t*rx1_buf,uint8_t*rx2_buf, uint8_t*dma_buf_num);
void sbus_to_rc(struct xyz_data_t *data);
/**************************************************************************/
uint8_t buf;
...
void sbus_to_rc(struct xyz_data_t *data)
{
	...
	if(un_2 == 1)
	{
		...
		if(count_t>=4)
		{
			...
			un_1 = 0;
			count_t = 0;
			abc.x  = ((tdata[0]<<8)|tdata[1])-320;
			abc.y = ((tdata[2]<<8)|tdata[3])-240;
		}
	}
}
